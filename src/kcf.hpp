...
{
public:
	KCF(std::string kernel_type = "gaussian", std::string feature_type = "gray");
...
protected:
	...
	inline cv::Size FloorSizeScale(cv::Size sz,double scale_factor) {
		return cv::Size(cvFloor(sz.width * scale_factor), cvFloor(sz.height * scale_factor));
	}

	inline cv::Point FloorPointScale(cv::Point p,double scale_factor) {
		return cv::Point(cvFloor(p.x * scale_factor), cvFloor(p.y * scale_factor));
	}
